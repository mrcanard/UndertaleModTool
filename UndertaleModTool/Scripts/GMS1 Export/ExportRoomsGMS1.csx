...
void DumpRoom(UndertaleRoom room)
{
    using (StreamWriter writer = new StreamWriter(roomsFolder + room.Name.Content + ".room.gmx"))
    {
        ...
        writer.WriteLine(
            "  <clearDisplayBuffer>" + (flagClearDisplayBuffer ? -1 : 0) + "</clearDisplayBuffer>"
        );
        ...
        foreach (var b in room.Backgrounds)
        {
            writer.WriteLine(
                "    <background visible=\""
                    ...
                    + (b.Foreground ? -1 : 0)
                    + "\" name=\""
                    + (b.BackgroundDefinition == null ? "" : b.BackgroundDefinition.Name.Content)
                    + "\" x=\""
                    + b.X
                    + "\" y=\""
                    + b.Y
        ...
        foreach (var g in room.GameObjects)
        {
            writer.WriteLine(
                "    <instance objName=\""
                    + (g.ObjectDefinition == null ? "undefined" : g.ObjectDefinition.Name.Content)
                    + "\" x=\""
                    + g.X
                    + "\" y=\""
                    + g.Y
